...
if [ $type -eq 0 ]; then
    file_name="./data/iperf_result/flow/${scale_num}_numa.txt"
elif [ $type -eq 1 ]; then
    file_name="./data/iperf_result/port/${scale_num}_numa.txt"
else
...
for i in {1..5}; do
    ...
    ip netns exec ns2 numactl --cpunodebind=0 --membind=0 iperf -c 192.100.1.2 -t 60 -i 1 >> $file_name
