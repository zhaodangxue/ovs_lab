...
* `Dockerfile`:利用Docker容器的方式帮助你快速搭建实验环境(包括上面的一些测试工具的安装），也可以利用如下指令快速拉取实验镜像，但是需要注意的是docker容器并不能帮助你模拟两个`numa`节点的情况，如果想要进行完整的本实验，还是应该使用多个`numa`节点的服务器。

  ```bash
  docker pull chairandsprite/ovs_lab:v1.2
  ```
...
1. **实验环境准备**
   ...
   - 相关软件的安装。在`linux ubuntu`操作系统下，执行如下指令进行对应工具的安装。(如果你是以`Dockerfile`构建对应的docker容器可以忽略这一步)
